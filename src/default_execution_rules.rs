...
impl BasicTurtleRegister {
        ...
        T: BasicTurtle<I, F, P, Error = E>,
    {
        if let Some(token) = &self.pop {
            ...
            executor.register_execution_rule(token.into(), move |s: &mut S| getter(s).pop());
        }
        if let Some(token) = &self.push {
            let getter = getter.clone();
            executor.register_execution_rule(token.into(), move |s: &mut S| getter(s).push());
        }
        if let Some(token) = &self.rotate {
            let getter = getter.clone();
            executor
                .register_execution_rule(token.into(), move |s: &mut S, r: F| getter(s).rotate(r));
        }
        if let Some(token) = &self.travel {
            let getter = getter.clone();
            executor
                .register_execution_rule(token.into(), move |s: &mut S, d: I| getter(s).travel(d));
        }
        if let Some(token) = &self.flush {
            let getter = getter.clone();
            executor
                .register_execution_rule(token.into(), move |s: &mut S| getter(s).flush().unwrap());
        }
    }
}
...
impl FancyTurtleRegister {
        ...
        T: FancyTurtle<I, F, P, C, Error = E>,
    {
        ...
        if let Some(token) = &self.set_color {
            let getter = getter.clone();
            executor.register_execution_rule(token.into(), move |s: &mut S, c: C| {
        ...
        if let Some(token) = &self.set_line_width {
            let getter = getter.clone();
            executor.register_execution_rule(token.into(), move |s: &mut S, w: F| {
